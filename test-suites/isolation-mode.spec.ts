...
makeSuite('Isolation mode', (testEnv: TestEnv) => {
  ...
  before(async () => {
    const { configurator, dai, usdc, aave, users, poolAdmin, aaveOracle, addressesProvider } =
      testEnv;
  ...
  it('Perform liquidation of isolation mode asset', async () => {
    ...
    // await aaveOracle.setAssetPrice(dai.address, daiPrice.mul(10));
    const priceId = await aaveOracle.getPriceFeedIdOfAsset(dai.address);
    await pyth.setPrice(priceId, daiPrice.mul(10));
  ...
  it('User 5 supplies isolation mode asset is liquidated by User 6', async () => {
    const { dai, aave, users, pool, helpersContract, aaveOracle } = testEnv;
