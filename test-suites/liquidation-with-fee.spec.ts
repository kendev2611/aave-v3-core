...
import { waitForTx, increaseTime, evmSnapshot, evmRevert, aave } from '@aave/deploy-v3';
...
makeSuite('Pool Liquidation: Add fee to liquidations', (testEnv) => {
  ...
  it('position should be liquidated when turn on liquidation protocol fee.', async () => {
    ...
    // await aaveOracle.setAssetPrice(usdc.address, '8000000000000000'); //weth = 500 usdc
    const usdcPriceId = await aaveOracle.getPriceFeedIdOfAsset(usdc.address);
    await pyth.setPrice(usdcPriceId, '8000000000000000');
  ...
  it('Drop the health factor below 1', async () => {
    ...
    await pyth.setPrice(daiPriceId, daiPrice.percentMul(11800));
  ...
  it('User 3 deposits 1000 USDC, user 4 0.06775 WETH, user 4 borrows - drops HF, liquidates the borrow', async () => {
    ...
    const usdcPriceId = await aaveOracle.getPriceFeedIdOfAsset(usdc.address);
    await pyth.setPrice(usdcPriceId, usdcPrice.percentMul(11200));
  ...
  it('User 4 deposits 0.03 AAVE - drops HF, liquidates the AAVE, which results on a lower amount being liquidated', async () => {
    ...
    //drops HF below 1
    const usdcPriceId = await aaveOracle.getPriceFeedIdOfAsset(usdc.address);
    await pyth.setPrice(usdcPriceId, usdcPrice.percentMul(11400));
