...
import { aave, evmRevert, evmSnapshot, waitForTx } from '@aave/deploy-v3';
...
makeSuite('Pool Liquidation: Liquidates borrows in eMode with price change', (testEnv: TestEnv) => {
  ...
  it('Drop HF below 1', async () => {
    ...
    const daiPrice = await aaveOracle.getAssetPrice(dai.address);
    const daiPriceId = await aaveOracle.getPriceFeedIdOfAsset(dai.address);
    ...
    await pyth.setPrice(daiPriceId, (await aaveOracle.getAssetPrice(dai.address)).add(1));
    // await aaveOracle.setAssetPrice(dai.address, (await oracle.getAssetPrice(dai.address)).add(1));
  ...
  it('Liquidation of non-eMode collateral with eMode debt for user in EMode', async () => {
    ...
    const wethPriceId = await aaveOracle.getPriceFeedIdOfAsset(weth.address);
    await pyth.setPrice(wethPriceId, wethPrice.percentMul(9000));
  ...
  it('Liquidation of eMode collateral with eMode debt in EMode with custom price feed', async () => {
    ...
    const EMODE_ORACLE_ADDRESS_PRICE_ID = ethers.utils.keccak256(user1.address);
    await aaveOracle.setAssetPriceFeedIds([user1.address], [EMODE_ORACLE_ADDRESS_PRICE_ID]);
    await pyth.setPrice(EMODE_ORACLE_ADDRESS_PRICE_ID, utils.parseUnits('1', 8));

    const daiPriceId = await aaveOracle.getPriceFeedIdOfAsset(dai.address);
    await pyth.setPrice(daiPriceId, utils.parseUnits('0.99', 8));

    const usdcPriceId = await aaveOracle.getPriceFeedIdOfAsset(usdc.address);
    await pyth.setPrice(usdcPriceId, utils.parseUnits('1.01', 8));

    const wethPriceId = await aaveOracle.getPriceFeedIdOfAsset(weth.address);
    await pyth.setPrice(wethPriceId, utils.parseUnits('4000', 8));
    ...
    await pyth.setPrice(EMODE_ORACLE_ADDRESS_PRICE_ID, oraclePrice.mul(2));
  ...
  it('Liquidation of non-eMode collateral with eMode debt in eMode with custom price feed', async () => {
    ...
    await pyth.setPrice(EMODE_ORACLE_ADDRESS_PRICE_ID, oraclePrice.mul(2));
    // await a.setAssetPrice(EMODE_ORACLE_ADDRESS, oraclePrice.mul(2));
